...
function pcm_edit_collection_page() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Decode the JSON string and sanitize each filter
        $applied_filters = isset($_POST['applied_filters']) ? json_decode(stripslashes($_POST['applied_filters']), true) : array();
        $applied_filters = array_map('sanitize_text_field', $applied_filters);
    ...
    $collection_data = isset($collections[$collection_name]) ? $collections[$collection_name] : array('description' => '', 'products' => array(), 'filters' => array());
        ...
        <form method="post">
            ...
            <div class="product-lists-container">
                ...
                <div class="product-list-wrapper">
                    ...
                    <div id="selected-product-list" class="product-grid">
                        <!-- Selected products will be populated here -->
                    </div>
                </div>
            </div>

            <input type="hidden" id="applied_filters" name="applied_filters" value="">
    ...
    var savedProducts = <?php echo json_encode($collection_data['products']); ?>;
    var savedFilters = <?php echo json_encode($collection_data['filters']); ?>;
    </script>
