...
class PCM_Elementor_Widget extends \Elementor\Widget_Base {
    ...
    protected function _register_controls() {
        ...
        // Top Rated Products Option
        $this->add_control(
            'show_top_rated_first',
            [
                'label' => __('Show Top Rated Products First', 'pcm'),
    ...
    protected function render() {
        ...
        $rows = $settings['rows'];
        $show_top_rated_first = $settings['show_top_rated_first'];
        ...
        // Sort products by rating if the option is enabled
        if ($show_top_rated_first === 'yes') {
            $product_ids = $this->sort_products_by_rating($product_ids);
        }
    
        $products_to_show = array_slice($product_ids, 0, $columns * $rows);
    
        echo '<div class="pcm-product-grid" style="display: grid; grid-template-columns: repeat(' . $columns . ', 1fr); gap: 20px;" data-show-top-rated-first="' . $show_top_rated_first . '">';
        foreach ($products_to_show as $product_id) {
            ...
            echo '<a href="' . esc_url($product_url) . '" class="pcm-product-card-link">';
    ...
    private function sort_products_by_rating($product_ids) {
        $products_with_ratings = array();

        foreach ($product_ids as $product_id) {
            $product = wc_get_product($product_id);
            if ($product) {
                $products_with_ratings[$product_id] = $product->get_average_rating();
            }
        }

        arsort($products_with_ratings);

        return array_keys($products_with_ratings);
    }
    ...
    protected function _content_template() {
        ...
        <# } else { #>
            <div class="pcm-product-grid" style="display: grid; grid-template-columns: repeat({{ settings.columns }}, 1fr); gap: 20px;" data-show-top-rated-first="{{ settings.show_top_rated_first }}">
                <# for (var i = 0; i < settings.columns * settings.rows; i++) { #>
                    <a href="#" class="pcm-product-card-link">
